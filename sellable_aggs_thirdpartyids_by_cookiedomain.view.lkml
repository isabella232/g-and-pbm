view: sellable_aggs_thirdpartyids_by_cookiedomain {
  derived_table: {
    sql: WITH AGG AS (SELECT cookiedomain, sum(cookie_count) third_party_IDs
      FROM no_id_logs.sellable_pairs_cookie
      WHERE event_date = (SELECT max(event_date) FROM no_id_logs.sellable_pairs_cookie)
      GROUP BY 1)
      ,bidder_sync_names AS (
      ...
      SELECT
        ...
        COALESCE(SUM(AGG.third_party_IDs ), 0) AS "unique_hems_by_cookie_domain.total"
      FROM AGG
  ...
  measure: unique_hems_by_cookie_domain_total {
    type: sum
    sql: ${TABLE}."unique_hems_by_cookie_domain.total" ;;
  }

  set: detail {
    fields: [bidder_sync_names_name, unique_hems_by_cookie_domain_total]
  }
}
