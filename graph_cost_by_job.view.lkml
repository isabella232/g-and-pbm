view: graph_cost_by_job {
  derived_table: {
    sql:  with spark_job as (
            ...
            SUM(COALESCE(counter.executor_run_time/1000000000,0)) exec_run_time_sec,
            SUM(COALESCE(counter.executor_cpu_time/1000000000,0)) exec_cpu_time_sec
            FROM metrics.spark
            WHERE type <> 'app'
            GROUP BY 1,2
          ),
            ...
            SUM(spark_job.exec_run_time_sec) run_time_sec,
        ...
        cluster_cost.cost * (jobs.run_time_sec/CAST(SUM(jobs.run_time_sec) OVER (PARTITION BY jobs.event_date) AS REAL)) cost_run_time,
        cluster_cost.cost * (jobs.data_mb/CAST(SUM(jobs.data_mb) OVER (PARTITION BY jobs.event_date) AS REAL)) cost_data_mb
        FROM (
