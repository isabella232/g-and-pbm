view: sellable_aggs_prob_partner_mapped {
  derived_table: {
    sql: WITH unique_pairs_by_cookie_domain AS (
      SELECT cookiedomain, probabilistic bucket, SUM(probabilistic_count) probabilistic_count
      ...
      SELECT
        ...
        COALESCE(SUM(unique_pairs_by_cookie_domain.probabilistic_count ), 0) AS "unique_pairs_by_cookie_domain.probabilistic"
 ...
 ;;
  ...
  measure: unique_pairs_by_cookie_domain_probabilistic {
    type: sum
    sql: ${TABLE}."unique_pairs_by_cookie_domain.probabilistic" ;;
  }

  set: detail {
    fields: [bidder_sync_names_name, unique_pairs_by_cookie_domain_bucket, unique_pairs_by_cookie_domain_probabilistic]
  }
}
