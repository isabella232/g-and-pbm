view: sellable_aggs_latest_date_partner {
  derived_table: {
    sql: WITH AGG AS (SELECT cookiedomain, max(event_date) latest_date
      ...
      bidder_sync_names AS (SELECT r.pub_or_app_id, bidder_array[r.pub_or_app_id] name
                  ...
                  SELECT
                    ...
                    AGG.latest_date AS "AGG.latest_date"
                  FROM AGG
  ...
  dimension: agg_latest_date {
    type: string
    sql: ${TABLE}."AGG.latest_date" ;;
  }

  set: detail {
    fields: [bidder_sync_names_name, agg_latest_date]
  }
}
