view: sellable_aggs_deterministic_partner_mapped {
  derived_table: {
    sql: WITH unique_pairs_by_cookie_domain AS (
      SELECT cookiedomain, deterministic bucket, SUM(deterministic_count) deterministic_count
      ...
      SELECT
        ...
        COALESCE(SUM(unique_pairs_by_cookie_domain.deterministic_count ), 0) AS "unique_pairs_by_cookie_domain.total"
 ...
 ;;
  ...
  set: detail {
    fields: [bidder_sync_names_name, unique_pairs_by_cookie_domain_bucket, unique_pairs_by_cookie_domain_total]
  }
}
