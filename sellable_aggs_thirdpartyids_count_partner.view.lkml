view: sellable_aggs_thirdpartyids_count_partner {
  derived_table: {
    sql: WITH MAG AS (WITH AGG AS (SELECT cookiedomain, max(event_date) latest_date
      FROM no_id_logs.sellable_pairs_cookie
      GROUP BY 1)

      SELECT t1.cookiedomain, sum(t1.cookie_count) third_party_IDs
      FROM no_id_logs.sellable_pairs_cookie t1
      ...
      bidder_sync_names AS (SELECT r.pub_or_app_id, bidder_array[r.pub_or_app_id] name
                  ...
                  SELECT
                    ...
                    COALESCE(SUM(MAG.third_party_IDs), 0) AS "unique_pairs_by_cookie_domain.thirdpartyIDs"
                  FROM MAG
  ...
  measure: unique_pairs_by_cookie_domain_thirdparty_ids {
    type: sum
    sql: ${TABLE}."unique_pairs_by_cookie_domain.thirdpartyIDs" ;;
  }

  set: detail {
    fields: [bidder_sync_names_name, unique_pairs_by_cookie_domain_thirdparty_ids]
  }
}
