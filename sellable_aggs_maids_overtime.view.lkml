view: sellable_aggs_maids_overtime {
  derived_table: {
    sql: WITH unique_pairs_by_cookie_domain AS (
      SELECT * FROM no_id_logs.sellable_pairs_cookie) ,
      ...
      SELECT
        ...
        COALESCE(SUM(unique_pairs_by_cookie_domain.cookie_count), 0) AS "unique_pairs_by_cookie_domain.total"
