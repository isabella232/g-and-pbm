view: liveramp_pairs_submitted_publisher {
  derived_table: {
    sql: SELECT event_date, publisher_id, COUNT(DISTINCT emailHash_pair) distinct_pairings_submitted,
...
(SELECT * FROM
(SELECT
PARSE_DATE('%Y%m%d',date) date, 'sync' as log_type, publisher_id,
...
UNION ALL
(SELECT
PARSE_DATE('%Y%m%d',date) date, 'track' as log_type, publisher_id,
...
datagroup_trigger: new_sync_day
  ...
  dimension: is_same_day_of_current_month {
    type: yesno
    sql: ${event_day_of_month} = EXTRACT(DAY FROM DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)) ;;
  }
